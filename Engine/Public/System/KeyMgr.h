...
{
	if (GameInstance()->Get_DIKeyState(iKey))
		return true;
...
bool CKeyMgr::Key_Down(const int& iKey)
{
	...
	if (!m_bKeyState[iKey] && (GameInstance()->Get_DIKeyState(iKey)))
		return true;
...
bool CKeyMgr::Mouse_Pressing(const MOUSEKEYSTATE& iMouse)
{
	...
	{
		if (GameInstance()->Get_DIMouseState((Engine::MOUSEKEYSTATE)iMouse))
			return true;
	}
	else if (iMouse >= DIM_MB && iMouse < DIM_END)
	{
		...
		case Engine::DIM_MWU:
		{
			if ((GameInstance()->Get_DIMouseMove(DIMS_Z) > 0L))
				return true;
			break;
		}
		case Engine::DIM_MWD:
...
bool CKeyMgr::Mouse_Down(const MOUSEKEYSTATE& iMouse)
{
	if (iMouse >= 0L && iMouse <= DIM_MB)
	{
		if (!m_bMouseState[iMouse] && GameInstance()->Get_DIMouseState((Engine::MOUSEKEYSTATE)iMouse))
			return true;
	}
	else if (iMouse >= DIM_MB && iMouse < DIM_END)
	{
		...
		case Engine::DIM_MWU:
		{
			if (!m_bMouseState[iMouse] && (GameInstance()->Get_DIMouseMove(DIMS_Z) > 0L))
				return true;
			break;
		}
		case Engine::DIM_MWD:
