...
CUI_Blink::CUI_Blink()
{
	Set_Name(TEXT("UI_Blink"));
}

CUI_Blink::CUI_Blink(const CUI_Blink& rhs)
	: Base(rhs)
{
}

HRESULT CUI_Blink::Initialize_Prototype()
{
...
HRESULT CUI_Blink::Initialize(void* Arg)
{
	return S_OK;
}

void CUI_Blink::Priority_Tick(const _float& fTimeDelta)
{
	SUPER::Priority_Tick(fTimeDelta);

}

void CUI_Blink::Tick(const _float& fTimeDelta)
{
	SUPER::Tick(fTimeDelta);

	m_State_Act.Get_StateFunc()(this, fTimeDelta);

	if (m_fBlink.Increase(fTimeDelta))
	{
		m_fBlink.Reset();
...
void CUI_Blink::Late_Tick(const _float& fTimeDelta)
{
	SUPER::Late_Tick(fTimeDelta);

}

HRESULT CUI_Blink::Render()
{
	SUPER::Render();
...
CUI_Blink* CUI_Blink::Create()
{
	ThisClass* pInstance = new ThisClass();

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		MSG_BOX("CUI_Blink Create Failed");
...
CGameObject* CUI_Blink::Clone(void* Arg)
{
...
FSerialData CUI_Blink::SerializeData()
{
	return FSerialData();
}

HRESULT CUI_Blink::Initialize_Component()
{
	// 프로토타입용 컴포넌트 초기화
	...
	m_pPlaneModelComp->TextureComp()->Bind_Texture(TEXT("Textures/RockmanDash2/UI/FadeOut/WhiteScreen.png"), 1);
...
HRESULT CUI_Blink::Bind_ShaderResources()
{
	if (m_pPlaneModelComp && m_bIsBlink)
	{
		auto pEffect = m_pPlaneModelComp->EffectComp();
		if (pEffect)
		{
			_float4x4 TempFloat4x4 = Transform().Get_TransformFloat4x4();
			_float TempFloat = 0.1f;
			_float2 TempFloat2 = {};
...
void CUI_Blink::ActState_Idle(const _float& fTimeDelta)
{
...
void CUI_Blink::ActState_FadeOut(const _float& fTimeDelta)
{
	if (m_State_Act.IsState_Entered())
	{
	...
	if (m_State_Act.Can_Update())
	{
		if (m_fAlpha.Increase(10.f * fTimeDelta))
		{
			Set_Dead();
		}
	}
